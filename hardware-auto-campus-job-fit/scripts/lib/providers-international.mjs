...
export function normalizeWorkday(raw,source,record){const d=raw.jobPostingInfo||{},parts=combinedBody(d.jobDescription);const loc=[d.location,...(d.additionalLocations||[]).map(x=>typeof x==='string'?x:x.descriptor)].filter(Boolean);return {
...
export function normalizeSmartRecruiters(d,source,record){const s=d.jobAd?.sections||{};let description=clean(s.jobDescription?.text),requirements=clean(s.qualifications?.text),body_complete=description.length>30&&requirements.length>25;if(!description||!requirements){const parts=combinedBody(description||requirements);description=parts.description;requirements=parts.requirements;body_complete=parts.body_complete;}const types=[d.typeOfEmployment?.label,d.experienceLevel?.label].filter(Boolean).join(' '),loc=[d.location?.city,d.location?.region].filter(Boolean);return {
  ...
  formal_status:formalType(d.name,description+'\n'+requirements,types),open_status:d.postingUrl?'open':'unknown',
...
export async function collectInternational(source,options={}){
  ...
  const client=createClient(opts),jobs=[],rows=new Map(),pages=[],errors=[];let total=null,complete=false,reason='max_pages_reached',filter=null;
  ...
  try{
    if(source.provider==='workday'){
      ...
      filter=chinaFacet(boot.data.facets);if(!filter)throw Error('No explicit mainland China country facet; cannot claim China coverage');
    ...
    for(let page=0;page<opts.maxPages;page++){
      ...
      for(const j of a)if(j.externalPath||j.id)rows.set(String(j.externalPath||j.id),j);
    ...
    await Promise.all(Array.from({length:opts.detailConcurrency},async()=>{while(cursor<queue.length){const row=queue[cursor++];try{
      ...
      if(!/^(cn|China|China Mainland|Mainland China|中国|中国大陆)$/i.test(country||'')){errors.push('Country filter not confirmed in detail '+(row.id||row.externalPath));continue;}
      const j=source.provider==='workday'?normalizeWorkday(r.data,source,r.record):normalizeSmartRecruiters(r.data,source,r.record);
