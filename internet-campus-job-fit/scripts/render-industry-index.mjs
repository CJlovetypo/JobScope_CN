...
let output=`# 行业公司索引\n\n截至 ${updated}，共 **${registry.companies.length} 个公司／招聘主体**，覆盖 ${INDUSTRIES.length} 个行业宽类。跨行业主体在运行时按公司 ID 去重。\n\n行业先分流，再按保存的城市标签硬筛。没有已确认城市不等于没有招聘；不使用总部地址填补岗位地点。接口可访问也不等于当前有正式校招。\n\n来源包括历年 WPS 校招汇总表及本轮用户指定的飞书多维表格；只有匿名 API 能取得完整 JD 的来源才会进入固定名单。原始表格、全部链接和逐接口证据保存在 \`artifacts/wps-campus-sources-20260919/\` 和 \`artifacts/feishu-source-expansion-20260919/\`。\n\n`;
