...
function official(key, j, source, context) {
  ...
  const direct = {
    ...
    meituan: `https://zhaopin.meituan.com/web/position/detail?jobUnionId=${id}&jobShareType=1&highlightType=campus`,
    ...
    mihoyo: `https://jobs.mihoyo.com/#/campus/position/${id}`,
  }[key];
...
export function normalizeCustomJob(key, j, source, context = {}) {
  ...
  switch (key) {
    ...
    case 'meituan':
      ...
      formal = str(j.jobType) === '1' ? 'formal' : str(j.jobType) === '2' ? 'internship' : 'social';
    ...
    case 'pdd':
      take(j.id, j.name, j.workLocationName || j.workLocation, ...combined(j.jobDuty)); oneBody = true;
    ...
    case 'shein':
      ...
      formal = j.jobTypeId === 'CAMPUS' && !intern.test(title) ? 'formal' : intern.test(j.jobTypeId || '') ? 'internship' : 'unknown'; break;
    case 'ctrip':
      ...
      formal = str(j.category) === '2' && (str(j.kind) === '1' || /Fresh Graduates/i.test(j.kindName || '')) ? 'formal' : /intern|实习/i.test(j.kindName || '') ? 'internship' : 'unknown'; break;
    case 'mihoyo':
      ...
      formal = j.jobNature === '实习' ? 'internship' : j.jobNature === '全职' && (/校招|秋招|春招/.test(j.projectName || '') || j.hireType === 1) ? 'formal' : 'unknown';
...
export async function collectCustom(source, options = {}) {
  ...
  if (!pair) return null;
  const [key, config] = pair, client = createClient(options), pages = [], all = new Map(), streams = [], failures = [];
  ...
  try {
    ...
    for (const q of config.discovery_requests) {
      ...
      if (key === 'tencent') {
        context.projectMappings = (res.data.data || []).flatMap(x => (x.subProjectList || []).filter(y => y.status === 1 && !/实习/.test(y.projectName) && (y.recruitType === 1 || /应届|培训生/.test(y.projectName))).map(y => ({ ...y })));
        ...
        const grad = (res.data.data || []).filter(x => /应届/.test(x.name));
    ...
    const pending = [...all.values()].filter(({ job }) => job.open_status !== 'closed' && !['internship', 'social'].includes(job.formal_status)
    ...
    await Promise.all(Array.from({ length: Math.min(3, pending.length) }, async () => {
      while (next < pending.length) {
        ...
        try {
          ...
          const res = await request(q, 'job_detail'), raw = res.data.data ?? res.data.body;
  ...
  const jobs = [...all.values()].map(x => x.job);
  const unknown = jobs.filter(j => j.formal_status === 'unknown' && j.open_status !== 'closed').length;
  const incomplete = options.mode === 'full' ? jobs.filter(j => j.formal_status === 'formal' && j.open_status !== 'closed' && cityMatch(j, options.cities) && !j.body_complete).length : 0;
  if (unknown) failures.push(`${unknown} published jobs have unconfirmed recruitment type; not classified as formal`);
  if (incomplete) failures.push(`${incomplete} formal jobs lack a complete JD`);
