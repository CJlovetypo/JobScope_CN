...
test('new reviewed companies have complete-JD proofs and matching metadata entries',async()=>{
 const sources=(await read('assets/sources.json')).companies,proof=[...(await read('data/source-verification-full-review-20260917.json')).configurations,...JSON.parse(await fs.readFile(path.join(SKILL_ROOT,'../shared/job-search-core/data/source-verification-feishu-20260919.json'),'utf8')).configurations],validTags=new Set(INDUSTRIES.map(i=>i.id));
