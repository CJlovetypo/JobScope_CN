...
if(!isMainThread){
 ...
 const flags=Object.fromEntries(process.argv.slice(2).map(a=>a.replace(/^--/,'').split('='))),mode=flags.mode||'campus';if(!SKILLS[mode])throw Error('Unknown mode');
 const root=path.join(PACK_ROOT,SKILLS[mode]),out=path.join(root,'artifacts/city-refresh-20260919'),cityFile=path.join(root,'data/company-city-index.json');
