...
- 从旧项目 `assets/company_registry.sqlite` 的公司和招聘入口取得候选，旧库只读。供应商行业宽类只用于发现线索，不是已核实的主营业务、公司性质或当前工作城市。
...
旧库全量复核见 [全量来源复核](../data/全量来源复核.md) 和 [旧库公司逐条核验](../data/旧库公司逐条核验-20260918.csv)，最新补漏闭环见 [第四轮闭环复核报告](../artifacts/wps-campus-sources-20260919/deep-api-review/第四轮闭环复核报告.md)，公司查询见 [行业公司索引](../data/行业公司索引.md)。旧库公司记录可能包含品牌、部门、活动与集团重复名；逐条有结论不等于每条都对应独立可用 API。未来维护时以实际文件与当次验证日期为准。
