...
- 筛选完成后先确定用户要实验批次、指定公司还是全量评估，再按已选范围使用双向匹配模型与能力证据模型，支持断点续跑。
- 最终 Excel 只包含岗位匹配、待核实与未评估、来源覆盖及说明；资料复核运行另有复核结果表。
- JD 全文单独存为 JSONL 归档。`runs/<运行名>/` 保存过程文件，`outputs/<运行名>/` 只保存最终交付文件。
