...
function enrich(job,source) {const loc=normalizeJobLocations(job);return {...job,company_id:source.company_id,company_name:source.display_name,cities:loc.cities,location_special:loc.special,location_unresolved:loc.unresolved,location_unknown:loc.unknown,location_code_evidence:loc.code_evidence,location_description_evidence:loc.description_evidence,locations_raw:loc.raw};}
...
function applyJobScope(result,filters) {
 ...
 for(const j of result.jobs){
  ...
  if(j.formal_status==='internship'||j.formal_status==='social'){j.evaluation_status='excluded_nonformal';counts.nonformal++;}
...
async function prepare() {
 ...
 if(flags['reuse-run']){
  ...
  for(const c of companies.filter(c=>c.selected)){
   ...
   if(old)await writeJson(path.join(dir,'companies',c.company_id+'.json'),applyJobScope(old,profile.city_filters));
  }
 }
...
async function collectRun() {
 ...
 await mapLimit(selected,integer('concurrency',3,8),async(c)=>{
  ...
  if(existing&&!flags.refresh&&existing.coverage.status==='complete'&&!existing.jobs.some(j=>j.city_status==='included'&&!j.body_complete&&!['internship','social'].includes(j.formal_status)&&j.open_status!=='closed'))return;
 ...
 run.status='collected';run.collected_at=new Date().toISOString();await writeJson(file,run);await nextBatch(dir);
}
async function nextBatch(dir) {
 dir=workspacePath(dir||path.resolve(String(flags.run||'')));const run=await readJson(path.join(dir,'run.json'));const pending=[];let total=0,done=0;
 for(const c of run.companies.filter(c=>c.selected)){
  ...
  for(const job of result.jobs.filter(j=>j.evaluation_status==='to_assess')){total++;job.jd_fingerprint=jobFingerprint(job);const reason=reviewNeedsUpdate(byId.get(String(job.job_id)),job,run.profile);if(!reason){done++;continue;}if(pending.length<integer('limit',20,100))pending.push({company:c,job,assessment_reason:reason});}
 }
 const file=path.join(dir,'next-batch.json');await writeJson(file,{
  ...
  evaluation_contract:{
   ...
   action_fields:'必填next_action(apply/verify/prepare/hold)、priority、priority_reason；high另需timing_evidence。按下一步行动紧迫性排序，不按匹配等级自动提档或降档。资格未确认符合或意愿冲突/未知不可直接投递。',
  ...
  },pending,total_to_assess:total,already_assessed:done,remaining:total-done
 });console.log(JSON.stringify({batch:file,items:pending.length,total,done,remaining:total-done}));
}
async function main(){
