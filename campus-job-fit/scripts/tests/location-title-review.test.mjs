...
const oldCompanies=new URL('../../runs/20260912-zhouyue-project-management/companies/',import.meta.url);
...
test('可用时只读复核旧运行全部待核实，亚马逊281条JSON地点全部还原',{
  skip:!fs.existsSync(oldCompanies),
},t=>{
  let reviewed=0,originalUnknown=0,restored=0,remaining=0,amazon=0;
  for(const file of fs.readdirSync(oldCompanies).filter(f=>f.endsWith('.json'))){
    const data=JSON.parse(fs.readFileSync(new URL(file,oldCompanies),'utf8'));
    for(const job of data.jobs.filter(j=>['needs_verification','missing_body'].includes(j.evaluation_status))){
      reviewed++;
      const r=normalizeJobLocations(job);
      const status=jobCityStatus({cities:r.cities,location_unknown:r.unknown,location_special:r.special},[]);
      if(job.city_status==='unknown')originalUnknown++;
      if(status==='unknown')remaining++;
      if(job.city_status==='unknown'&&status!=='unknown')restored++;
      if(job.company_name==='亚马逊中国'){
        amazon++;assert.ok(r.cities.length,job.job_id);assert.equal(r.unknown,false,job.job_id);
        assert.deepEqual(r.raw,job.locations_raw);
        assert.ok(r.structured_evidence.some(e=>e.source==='locations_raw_json'&&e.pointer&&e.raw),job.job_id);
      }
    }
  }
  assert.equal(reviewed,1927);assert.equal(originalUnknown,806);assert.equal(amazon,281);
  assert.ok(restored>=439);assert.equal(restored+remaining,originalUnknown);
  t.diagnostic(JSON.stringify({reviewed,originalUnknown,restored,remaining,amazon}));
});
