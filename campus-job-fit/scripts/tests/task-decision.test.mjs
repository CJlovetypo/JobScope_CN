...
for(const mode of ['campus','internship','social'])test(`real ${mode} CLI: discovery prepare, cached collect, report, no fabricated profile or personal assessment`,async()=>{
 const source=(await readSourceRegistry()).companies[0],root=path.join(PACK_ROOT,SKILLS[mode]),dir=path.join(root,'runs','discovery-test-'+randomUUID());
...
for(const mode of ['campus','internship','social'])test(`real ${mode} CLI: empty profile can prepare a v5 limited assessment without fabricated facts`,async()=>{
 const source=(await readSourceRegistry()).companies[0],dir=path.join(PACK_ROOT,SKILLS[mode],'runs','v5-empty-'+randomUUID());
 const input=path.join(dir,'input.json'),record=path.join(dir,'task.json');
 ...
 await write(path.join(dir,'companies',source.company_id+'.json'),{source_config_fingerprint:sourceConfigFingerprint(source,mode),coverage:{status:'complete'},jobs:[{job_id:'J1',company_id:source.company_id,title:'合成岗位',description:'跟踪进度',requirements:'本科',formal_status:mode==='campus'?'formal':mode,open_status:'open',body_complete:true,cities:['上海'],evaluation_status:'to_assess'}]});
 ...
 await cli('batch-close','--mode',mode,'--run',dir,'--batch',batch.batch_id);
});

test('new matching CLI binds scope and accepts only a continuous scope revision without losing old tasks',async()=>{
 const source=(await readSourceRegistry()).companies[0],dir=path.join(PACK_ROOT,'campus-job-fit/runs','scope-task-'+randomUUID());
