...
test('shared employer-size and direction-specific city indexes cover the current registry',async()=>{
 ...
 for(const skill of ['campus-job-fit','internship-job-fit','social-job-fit'])assert.deepEqual((await read(skill+'/data/company-city-index.json')).companies.map(c=>c.company_id).sort(),ids);
});
test('repair commits only unchanged expected config, preserves history and refuses concurrent replacement',async()=>{
 const folder=path.resolve('campus-job-fit/artifacts/implementation/tests/repair-'+Date.now()),file=path.join(folder,'registry.json');await fs.mkdir(folder,{recursive:true});await fs.writeFile(file,JSON.stringify({companies:[company]}));
