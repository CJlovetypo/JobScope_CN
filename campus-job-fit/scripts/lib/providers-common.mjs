...
function classify(d, provider, campusContext) {
  ...
  else if (campusContext === false && /^(全职|正式|full.?time)$/i.test(type)) formal = 'social';
...
function contextsFor(source) {
  ...
  for (const q of examples.filter(q => pattern.test(q.url))) {
    ...
    const isCampus = provider === 'moka' ? /campus/.test(b.site || entry) : provider === 'moka_api_platform' ? new URL(q.url).searchParams.get('mode') === 'campus'
      : provider === 'beisen' ? (b.Category || []).map(String).includes('2') : provider === 'hotjob' ? Number(b.recruitType) === 1 : true;
    contexts.set(key, { key, q, body: b, entry, origin, detail, isCampus });
  }
...
function normalize(d, provider, context, source, rawFile) {
  ...
  return { job_id: String(d.id || d.Id || d.postId || ''), company_id: source.company_id, company_name: source.display_name,
    ...
    ...classify(d, provider, context.isCampus), ...officialUrl(d, provider, context, source), raw_file: rawFile,
    raw_metadata: { department: d.department || d.Org || d.orgName, project: d.job_subject || d.projectFolder || d.projectName,
...
async function collectContext(source, context, options, client) {
  ...
  try {
    ...
    if (provider !== 'feishu' && provider !== 'moka_api_platform') {
      ...
      if (boot.record.http_status !== 200) throw new Error(`Public bootstrap HTTP ${boot.record.http_status}`);
      if (provider === 'moka') {
    ...
    for (let page = 0; page < options.maxPages; page++) {
      ...
      if (provider === 'moka') body = { ...body, limit: size, offset, keyword: '', needStat: true };
        ...
        DisplayFields: ['Category', 'Kind', 'LocId', 'LocNames', 'Duty', 'Require', 'Status'] };
  ...
  const requiredIncomplete = options.mode === 'full' ? jobs.filter(j => !j.detail_skipped_reason && j.formal_status === 'formal' && j.open_status === 'open' && !j.body_complete).length : 0;
