...
export const RECRUITMENT_POLICY_VERSION = '2026-09-12-campus-evidence';
const yes = value => value === true || value === 1 || value === '1' || value === 'true';
const label = value => typeof value === 'string' ? value : value?.name?.zh_cn || value?.name?.i18n || value?.name || '';
const campusLabel = value => /校招|校园招聘|应届|fresh\s*graduate|university\s*(?:recruit|hiring|graduate)|graduate\s*(?:recruit|program|scheme)|campus\s*(?:recruit|hiring)/i.test(label(value));
...
export function campusEvidence(job) {
  ...
  if (yes(e.showIsCampus)) add('recruitment_evidence.showIsCampus',e.showIsCampus);
...
export function reviewRecruitment(job) {
  ...
  if (e.type_conflict || e.conflicting_recruitment_types?.length > 1) {
    ...
    reason = '尚未找到该岗位的校招类型标签或校招项目关联证据；仅全职、公司校招入口或初级岗位名称不足以确认';
  }
...
export function verificationIssues(job) {
  const result = [];
  if (job.formal_status !== 'formal') result.push({code:'recruitment',reason:job.recruitment_evidence?.admission_review?.reason || '该岗位的校招性质尚未确认'});
