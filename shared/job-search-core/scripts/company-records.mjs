...
import {loadCompanyInputs,buildCompanyRecords,researchQueue,validateReview,campaignProgress,REVIEW_FILE,indexById} from './lib/company-records.mjs';

const read=async p=>JSON.parse(await fs.readFile(p,'utf8'));
async function save(p,value,{exclusive=false}={}) {
  await fs.mkdir(path.dirname(p),{recursive:true});
  if(exclusive)return fs.writeFile(p,JSON.stringify(value,null,2)+'\n',{flag:'wx'});
  const tmp=p+'.'+randomUUID()+'.tmp';await fs.writeFile(tmp,JSON.stringify(value,null,2)+'\n');await fs.rename(tmp,p);
}
...
export async function main(argv=process.argv.slice(2)) {
  ...
  if(command==='export') {
    const file=opts.out?artifactPath(opts.out):path.join(CORE_ROOT,'data/company-records.json');
    const result=buildCompanyRecords(inputs);await save(file,result);console.log(JSON.stringify({file,companies:result.companies.length,operation:'structural_export_not_reverification'}));return;
  }
  if(command==='init') {
  ...
  if(command==='publish') {
    ...
    for(const review of reviews.companies) {
      ...
      if(old&&old.campaign_id!==review.campaign_id&&Date.parse(old.searches?.[0]?.searched_at)>Date.parse(campaign.started_at))throw Error('存在更新批次结论，拒绝旧批次覆盖');
      previous.set(review.company_id,review);
    }
    ...
    await save(path.join(CORE_ROOT,'data/company-records.json'),buildCompanyRecords({...inputs,reviews:next}));
    await save(path.join(root,'progress.json'),progress);
