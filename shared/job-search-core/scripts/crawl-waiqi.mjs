...
for(let page=1;page<=Math.ceil(total/100);page++){
  const data=await request(`lists/${page}.json`,'/company/foreign/search',{cityId:'',businessDictIdList:[],companyTypeList:[],posInfoIdList:[],workExpList:[],educationList:[],page,size:100,needAd:0});
...
await Promise.all(Array.from({length:concurrency},async()=>{
  while(cursor<items.length){const row=items[cursor++];
    let detail;
    try{detail=await request(`companies/${row.id}.json`,`/company/info?id=${row.id}`);}catch{errors++;}
    ...
    try{await request(`positions/${row.id}.json`,`/company/position-all?companyId=${row.id}`,undefined,cached=>cached?.success&&cached.observation_kind!=='company_info_reports_zero_positions');}catch{errors++;}
...
const detailMode=process.env.WAIQI_JOB_DETAILS||'missing-links';
...
await write('crawl-summary.json',{finished_at:new Date().toISOString(),reported_total:total,companies:companies.size,errors,job_detail_mode:detailMode,job_details_requested:queue.length,job_detail_errors:detailErrors});
