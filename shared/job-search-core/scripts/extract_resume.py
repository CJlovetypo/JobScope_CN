...
def main(skill_root):
 ...
 p=Path(args.input).resolve();out=Path(args.out).resolve();skill=Path(skill_root).resolve()
...
if __name__=='__main__':raise SystemExit('请通过对应 skill 的 scripts/extract_resume.py 运行，以确定产物所属目录。')
