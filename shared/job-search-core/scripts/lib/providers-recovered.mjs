...
export async function collectRecovered(source,options={}) {
  if(!['51job_coapi','zhaopin_grace','greenhouse'].includes(source.provider))return null;
  if(source.provider==='greenhouse'&&!source.api_config?.mainland_location_pattern)throw Error('Greenhouse requires an observed mainland location pattern for this employer');
  ...
  try {
    for(let p=1;p<=opts.maxPages;p++) {
      ...
      if(source.provider==='greenhouse'){reason='single_endpoint_total_mismatch';break;}
    }
  ...
  return {company_id:source.company_id,display_name:source.display_name,checked_at:new Date().toISOString(),jobs,requests:client.records,
    coverage:{status:listComplete&&!errors.length?'complete':pages.length?'partial':'failed',mode:opts.mode,city_filters:cities,pages:pages.length,
      ...
      scope:source.provider==='greenhouse'?'Public global tenant enumerated; only postings with explicitly observed mainland locations retained':'Public employer tenant; recruitment type reviewed separately for each JD'}};
}
