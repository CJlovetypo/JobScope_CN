...
新增来源基本能力通过后，必须执行共享 [定向能力固定验收流程](../../../../shared/job-search-core/references/targeted-search.md#新增与修复来源的固定验收流程)。每个新增配置在三个招聘方向均须有验证结论，禁止以平台样本替代。完成正式写入后执行维护审计及 --check；未处理项不能遗漏，未证实项保留原因并使用本地筛选，不阻塞已合格的基础来源准入。
