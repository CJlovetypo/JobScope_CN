  ...
  for(let index=0;index<entries.length;index++){
    ...
    try {
      ...
      if(result.coverage.pages&&['beisen','feishu','hotjob'].includes(source.provider)){
        ...
        const cap=await collectCommon(broad,{...settings,maxPages:2,evidenceDir:path.join(attemptDir,'capability-http')});cap.jobs=cap.jobs.map(j=>reviewRecruitment(reviewJobBody(j)));
